...
static void spw_tx_callback(void *arg, const Spw_Tx_IrqStatus *const irqStatus)
{
    ...
    if (irqStatus->sendListDeactivatedIrqOccurred) {
        self->tx_done = true;
        rtems_status_code rc = rtems_semaphore_release(self->tx_semaphore);
        (void)rc;
    }
}
...
static void spw_rx_callback(void *arg, const Spw_Rx_IrqStatus *const irqStatus)
{
    ...
    if (irqStatus->receivedBufferDeactivatedIrqOccurred) {
        ...
        rtems_status_code rc = rtems_semaphore_release(self->rx_semaphore);
        (void)rc;
    }
}
...
static void process_rx_packets(samrh71_rtems_spw_private_data *const self)
{
    ...
    for (uint16_t i = 0U; i < prevStatus.receivedPackets; i++) {
        ...
        if (length > 0U) {
            Broker_receive_packet(self->ip_device_bus_id, data, length);
        }
    }
}

void init_spw_driver(samrh71_rtems_spw_private_data *const self, const uint8_t txInitDiv, const uint8_t txOperDiv = 0U)
{
    Spw_init(&g_spw);
    ...
    const Spw_TxHandler txHandler = { .callback = spw_tx_callback, .arg = self };
    const Spw_RxHandler rxHandler = { .callback = spw_rx_callback, .arg = self };
...
void init_rtems_synchronization_primitives(samrh71_rtems_spw_private_data *const self)
{
    self->tx_done       = false;
    self->rx_deactivated = false;
